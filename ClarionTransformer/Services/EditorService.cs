...
namespace ClarionTransformer.Services
{
    ...
    public class EditorService
    {
        ...
        /// <summary>
        /// Igual que <see cref="GetSelectedText"/> pero SIN Trim(). Necesario cuando el resultado
        /// se va a reinsertar por posicion (ver GetSelectionRange): si la seleccion arranca al
        /// principio de una linea con indentacion, Trim() se come esos espacios iniciales y la
        /// primera linea del texto enviado a la IA — y por lo tanto del resultado — queda sin
        /// indentar aunque las demas lineas si la conserven.
        /// </summary>
        public string GetSelectedTextRaw()
        {
            try
            {
                var textArea = GetActiveTextArea();
        ...
        /// <summary>
        /// Range of the current selection as {startLine, startCol, endLine, endCol}, 1-based,
        /// where endCol/endLine point just PAST the last selected character (same convention
        /// used by ReplaceRange/SelectRange). Returns null if there is no selection.
        /// Position-based, so it survives editing done elsewhere in the meantime — unlike
        /// text-content matching (ReplaceText), which breaks if the same text occurs more than
        /// once or line endings differ between the selection buffer and the document.
        /// </summary>
        public int[] GetSelectionRange()
        {
            try
            {
                ...
                var hasSelection = GetProperty(selMgr, "HasSomethingSelected");
                if (!(hasSelection is bool) || !(bool)hasSelection) return null;

                var selections = GetProperty(selMgr, "SelectionCollection") as System.Collections.IEnumerable;
                ...
                var startPos = GetProperty(first, "StartPosition");
                var endPos   = GetProperty(last, "EndPosition");
                if (startPos == null || endPos == null) return null;

                int startLine = (int)GetProperty(startPos, "Y") + 1;
                int startCol  = (int)GetProperty(startPos, "X") + 1;
                int endLine   = (int)GetProperty(endPos, "Y") + 1;
                int endCol    = (int)GetProperty(endPos, "X") + 1;
